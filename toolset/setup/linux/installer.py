...
class Installer:
  ...
  def __install_server_software(self):
    ...
    self.__run_command("sudo perl perl-build.pl -DDEBUGGING=-g 5.18.2 ~/FrameworkBenchmarks/installs/perl-5.18", retry=True)
    self.__download("http://cpanmin.us", "cpanminus.pl")
    self.__run_command("perl cpanminus.pl --notest --no-man-page --sudo App::cpanminus", retry=True)
    self.__run_command("cpanm -f --notest --no-man-page -S DBI DBD::mysql Kelp Dancer Mojolicious Kelp::Module::JSON::XS Dancer::Plugin::Database Starman Plack JSON Web::Simple DBD::Pg JSON::XS EV HTTP::Parser::XS Monoceros EV IO::Socket::IP IO::Socket::SSL", retry=True)
